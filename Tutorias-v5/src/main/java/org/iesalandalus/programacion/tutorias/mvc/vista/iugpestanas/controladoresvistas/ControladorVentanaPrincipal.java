...
/*Implementa Initializable y aÃ±ade el mÃ©todo initialize
 * 
 * Importar el IControlador y asignarlo
 * Crear ObservableList
 * En FactoriaVista aÃ±adir el IUGPESTANAS
 * 
 * 
 */

// CREARCITA EN PESTAÃ‘A ALUMNOS AL MENUITEM OOOOJOO

// EN PRINCIPIO ESTÃ� ACABADA----------------------------------------------------------
...
public class ControladorVentanaPrincipal implements Initializable {
	 ...
	 * "Â¿EstÃ¡s seguro de que quieres anular la reserva?"; private static final
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		tcAlumnosCitasProfesor
				.setCellValueFactory(cita -> new SimpleStringProperty(cita.getValue().getSesion().getTutoria().getProfesor().getNombre()));
	...
	@FXML
	private void salir() {
		if (Dialogos.mostrarDialogoConfirmacion("Salir", "Â¿EstÃ¡s seguro de que quieres salir de la aplicaciÃ³n?",
				null)) {
	...
	private void acercaDe() throws IOException {
		...
		Dialogos.mostrarDialogoInformacionPersonalizado("GestiÃ³n de TutorÃ­as", contenido);
	}
	...
	private void crearAnadirProfesor() throws IOException {
		if (anadirProfesor == null) {
			...
			anadirProfesor.setTitle("AÃ±adir Profesor");
	...
	private void crearAnadirAlumno() throws IOException {
		if (anadirAlumno == null) {
			...
			anadirAlumno.setTitle("AÃ±adir Alumno");
	...
	private void crearAnadirCitasDeLaSesion() throws IOException {
		...
		if (anadirCitaDeLaSesion == null) {
			...
			anadirCitaDeLaSesion.setTitle("AÃ±adir Cita de la SesiÃ³n");
	...
	private void crearAnadirSesion() throws IOException {
		...
		if (anadirSesion == null) {
			...
			anadirSesion.setTitle("AÃ±adir SesiÃ³n");
	...
	private void crearAnadirCitaAlumno() throws IOException {
		...
		if (anadirCitaAlumno == null) {
			...
			anadirCitaAlumno.setTitle("AÃ±adir Cita del Alumno");
	...
	private void crearAnadirTutoria() throws IOException {
		...
		if (anadirTutoria == null) {
			...
			anadirTutoria.setTitle("AÃ±adir tutorÃ­a");
	...
	void borrarProfesor(ActionEvent event) {
		...
		try {
			...
			if (profesor != null && Dialogos.mostrarDialogoConfirmacion(BORRAR_PROFESOR,
					"Â¿EstÃ¡s seguro de que quieres borrar el profesor?", null)) {
	...
	void borrarAlumno(ActionEvent event) {
		...
		try {
			...
			if (alumno != null && Dialogos.mostrarDialogoConfirmacion(BORRAR_ALUMNO,
					"Â¿EstÃ¡s seguro de que quieres borrar el alumno?", null)) {
	...
	void borrarTutoria(ActionEvent event) {
		...
		try {
			...
			if (tutoria != null && Dialogos.mostrarDialogoConfirmacion(BORRAR_TUTORIA,
					"Â¿EstÃ¡s seguro de que quieres borrar la tutorÃ­a?", null)) {
				controladorMVC.borrar(tutoria);
				tutorias.remove(tutoria);
				Dialogos.mostrarDialogoInformacion(BORRAR_TUTORIA, "TutorÃ­a borrada satisfactoriamente");
			}
		} catch (Exception e) {
	...
	void borrarCitaAlumno(ActionEvent event) {
		...
		try {
			...
			if (cita != null && Dialogos.mostrarDialogoConfirmacion(BORRAR_CITA,
					"Â¿EstÃ¡s seguro de que quieres borrar la cita?", null)) {
	...
	void borrarSesion(ActionEvent event) {
		...
		try {
			...
			if (sesion != null && Dialogos.mostrarDialogoConfirmacion(BORRAR_SESION,
					"Â¿EstÃ¡s seguro de que quieres borrar la sesiÃ³n?", null)) {
				controladorMVC.borrar(sesion);
				sesiones.remove(sesion);
				Dialogos.mostrarDialogoInformacion(BORRAR_SESION, "SesiÃ³n borrada satisfactoriamente");
			}
		} catch (Exception e) {
	...
	// ACTUALIZAR Â¿Es necesario? Revisar, creo que no se llama desde nigÃºn sitio
